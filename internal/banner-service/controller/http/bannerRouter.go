...
func SetupBannerRoutes(authenticator *authenticate.Authenticator, bannerUsecase *banner.BannerUsecase, imageUsecase *banner.BannerImageUsecase) http.Handler {
	...
	//IFrame
	muxRouter.Handle("/api/v1/banner/iframe/{banner_id:[0-9]+}", logger.LogMiddleware(authenticate.AuthMiddleware(authenticator)(http.HandlerFunc(bannerController.GetBannerIFrame)))).Methods("GET")
	// Работа с картинками
